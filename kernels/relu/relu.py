"""
relu.py —— 编译并基准测试 relu.sycl 中的 6 个 SYCL/oneAPI kernel

流程：
  1. 用 torch.utils.cpp_extension.load 现场编译 relu.sycl；
  ...
  4. 与 PyTorch 官方 torch.relu 对比正确性（打印前 2 个结果）和性能。

与 elementwise / sigmoid 模块的区别：relu 也是单输入算子，kernel 接口是 (x, y)，
只读一份输入数据；但每个元素只需一次取最大值，是计算极轻的逐元素算子。

与 CUDA 版 relu.py 保持一致的一点：官方对照直接调用 torch.relu（不像 sigmoid.py
那样用 partial(torch.sigmoid, out=y)），因此 out_*_th 的耗时里包含每次迭代新建
输出张量的开销。
...
# relu 不需要反向传播，关闭 autograd 可以省去图构建开销
...
lib = load(
    ...
    # 对照：CUDA 版 relu.py 还用了 -U__CUDA_NO_HALF_OPERATORS__ 等 half 运算符开关
    #      与 --use_fast_math；SYCL 侧由 SyclExtension 处理 half 类型，
    #      ReLU 本体只有一次取最大值，也用不到快速数学函数。
    extra_cflags=["-O3"],
    extra_sycl_cflags=["-O3"],
    verbose=False,
)

# 打印 PyTorch SYCL 扩展的实际构建目录（relu_lib.so 所在位置）
print(ext._get_build_directory("relu_lib", False))


# 基准测试封装：
# - 传入 out 时走“原地写 y”的 kernel 接口（本模块 kernel 签名是 (x, y)）；
# - 不传 out 时函数返回新张量（如 torch.relu 默认接口）；
...
def run_benchmark(
    ...
    # 左对齐补空格；LeetOneAPI 的 elementwise / sigmoid 模块统一去掉了这一步，
    # 本模块沿用同样的打印格式。
    # 随机张量前两个元素为负时会被 ReLU 截成 0，打印为 0.0 属正常现象。
...
for S, K in SKs:
    ...
    # torch.relu 没有 out 参数，每次迭代都会新建输出张量（照搬 CUDA 版口径）
    ...
    # 属于预期精度损失；ReLU 是“原样返回或清零”，误差不会被放大。
