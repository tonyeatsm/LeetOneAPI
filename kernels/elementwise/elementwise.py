"""
elementwise.py —— 编译并基准测试 elementwise.sycl 中的 6 个 SYCL/oneAPI kernel

流程：
  1. 用 torch.utils.cpp_extension.load 现场编译 elementwise.sycl；
...
# 打印 PyTorch SYCL 扩展的实际构建目录（elementwise_lib.so 所在位置）
...
for S, K in SKs:
    ...
    a = torch.randn((S, K)).xpu().float().contiguous()
    b = torch.randn((S, K)).xpu().float().contiguous()
    c = torch.zeros_like(a).xpu().float().contiguous()
    # 依次测：标量 / 4 元素向量化 / PyTorch 官方实现
    ...
    # FP16 尾数只有 10 位，最小刻度 2^-10 ≈ 0.00098（约 3~4 位有效数字，
    # 保守按约 3 位），FP32 转 half 会就近取整造成量化误差，属预期精度损失。
    # FP16 测试：从同一组随机数转成 half，便于对比精度损失
    a_f16 = a.half().contiguous()
    b_f16 = b.half().contiguous()
    c_f16 = c.half().contiguous()
    # 依次测：标量 / 2 元素 / 8 元素 / 128 位打包 / PyTorch 官方实现
...
print("elementwise benchmark done.")
